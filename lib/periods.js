...
Periods.prototype.getAllByScheduleID = function(scheduleID, callback) {
  this.collection.find({schedule_id: scheduleID}).sort({ number: 1 }).toArray(callback);
};
 ...
 * RFC 3339
 */
Periods.prototype.normalizeForDatetime = function(normalizedTime) {
  if (normalizedTime.length == 4) {
    return "0" + normalizedTime;
  } else {
